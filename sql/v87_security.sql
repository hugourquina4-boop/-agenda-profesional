-- v87: endurecimiento de seguridad
-- 1. Contraseña 8+ caracteres con al menos un dígito en salon_self_service_registrar
-- 2. Tabla rate_limits para frenar spam desde Edge Functions
-- 3. pg_cron: auto-limpiar admin_clave_temp después de 24h
-- 4. pg_cron: purgar registros viejos de rate_limits cada 30 min

-- ══════════════════════════════════════════════════════════════════════════════
-- 1. Actualizar salon_self_service_registrar con contraseña más fuerte
-- ══════════════════════════════════════════════════════════════════════════════
...
DECLARE
  ...
  v_nombre    TEXT;
BEGIN
  -- Validación contraseña: 8 chars mínimo + al menos un dígito
  IF length(trim(p_clave)) < 8 THEN
    RETURN jsonb_build_object('ok', false, 'error', 'clave_corta');
  END IF;
  IF trim(p_clave) !~ '[0-9]' THEN
    RETURN jsonb_build_object('ok', false, 'error', 'clave_sin_numero');
  END IF;
...
-- ══════════════════════════════════════════════════════════════════════════════
-- 2. Tabla rate_limits — usada por Edge Functions para limitar peticiones por IP
-- ══════════════════════════════════════════════════════════════════════════════
...
-- Sólo accesible vía service_role desde Edge Functions; sin RLS
ALTER TABLE rate_limits DISABLE ROW LEVEL SECURITY;
GRANT SELECT, INSERT, DELETE ON rate_limits TO service_role;

-- ══════════════════════════════════════════════════════════════════════════════
-- 3. pg_cron: borrar admin_clave_temp después de 24h (cada hora, minuto 17)
-- ══════════════════════════════════════════════════════════════════════════════
SELECT cron.schedule(
  'clear-admin-clave-temp',
  '17 * * * *',
  $$
    UPDATE tenants
       SET admin_clave_temp = NULL
     WHERE admin_clave_temp IS NOT NULL
       AND admin_clave_fecha < now() - interval '24 hours';
  $$
);

-- ══════════════════════════════════════════════════════════════════════════════
-- 4. pg_cron: purgar rate_limits > 2h (cada 30 min)
-- ══════════════════════════════════════════════════════════════════════════════
SELECT cron.schedule(
  'purge-rate-limits',
  '*/30 * * * *',
...
DO $$ BEGIN
  RAISE NOTICE '✓ v87: contraseña 8+dígito · rate_limits table · cron clear-admin-clave + purge-rate-limits';
END $$;
